...
(async () => {
    try {
        ...
        const videoUrl = Buffer.from(videoUrlEncoded, 'base64').toString('utf-8');
        const thumbnailUrl = thumbnailUrlEncoded ? Buffer.from(thumbnailUrlEncoded, 'base64').toString('utf-8') : undefined;
