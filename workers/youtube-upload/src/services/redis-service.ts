...
import { YoutubeUploadJob, JobStatus, JobProgress } from '../types';

const JOB_QUEUE_KEY = 'youtubeUploader:jobs:queue';
const JOB_DATA_PREFIX = 'youtubeUploader:job:';
const JOB_PROGRESS_PREFIX = 'youtubeUploader:progress:';
...
class RedisService {
     ...
     */
    async getNextJob(): Promise<YoutubeUploadJob | null> {
        try {
            ...
            return JSON.parse(jobData) as YoutubeUploadJob;
        } catch (error) {
    ...
    async updateJobProgress(
        ...
        progress: number,
        message: string,
        additionalData?: Partial<YoutubeUploadJob>
    ): Promise<void> {
        try {
            ...
            const job: YoutubeUploadJob = JSON.parse(jobData);

            // Update job
            const updatedJob: YoutubeUploadJob = {
                ...job,
                status,
                progress,
                message,
    ...
    async completeJob(
        jobId: string,
        results: Partial<YoutubeUploadJob>
    ): Promise<void> {
     ...
     * Get job by ID
     */
    async getJob(jobId: string): Promise<YoutubeUploadJob | null> {
        try {
            const jobData = await this.client.get(`${JOB_DATA_PREFIX}${jobId}`);
