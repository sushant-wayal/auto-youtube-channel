...
/**
 * Keyword Extraction Utility (Hardened)
 * Robust against broken / partial / messy AI JSON
 */
...
const STOPWORDS = new Set([
  'the','and','for','with','this','that','you','we','they','when','what','how','why','who',
  'which','where','there','here','from','but','can','will','would','could','should','may',
  'might','must','are','was','were','been','being','have','has','had','does','did','doing',
  'not','now','then','than','also','just','only','very','too','more','most','some','any',
  'all','into','out','over','under','about','after','before','between','through','during',
  'while','your','our','their','its','his','her','him','them','these','those','such','each',
  'own','same','few','many','much','other','another','both','either','neither','pause',
  'like','well','way','make','get','see','know','take','come','go','think','look','want',
  'use','find','give','tell','work','call','try','ask','need','feel','become','leave',
  'put','mean','keep','let','begin','seem',
]);
...
// Extracts the largest balanced JSON array it can find
function extractJsonArray(raw: string): string | null {
  ...
  // Unterminated array → auto-close
  return raw.slice(start) + ']';
}

// Normalizes common AI JSON sins
...
function safeJsonParse<T>(raw: string): T | null {
  ...
  const sanitized = sanitizeJsonLike(extracted);
...
function validateKeyPhrases(data: unknown[]): KeyPhrase[] {
  return data
    .filter(
      (x): x is { phrase: string; wordsCovered: number } =>
        typeof x === 'object' &&
        x !== null &&
        typeof (x as any).phrase === 'string' &&
        Number.isFinite(Number((x as any).wordsCovered))
    )
    .map(x => ({
      phrase: x.phrase.trim(),
      wordsCovered: Math.max(1, Math.round(Number(x.wordsCovered))),
    }));
}
...
export class KeywordExtractor {
  ...
  async extractKeywords(
    ...
    const cleanNarration = narration.replace(/\[PAUSE\]/g, '');
    const totalWords = cleanNarration.trim().split(/\s+/).length;
...
RULES:
- ${minClips} to ${maxClips} phrases (recommended ${recommendedClips})
- 3–4 word visual phrases
- wordsCovered must sum to ${totalWords}

FORMAT:
[
  { "phrase": "person working laptop", "wordsCovered": 20 }
]
`;
    ...
    try {
      ...
      let parsed = safeJsonParse<unknown[]>(response);

      // Second-pass repair if needed
      if (!parsed) {
        const repair = await this.geminiService.generateText(
          `Fix this into valid JSON ONLY:\n${response}`,
          { temperature: 0 }
        );
        parsed = safeJsonParse<unknown[]>(repair);
      }

      if (!parsed) throw new Error('Unrecoverable JSON');

      let keyPhrases = validateKeyPhrases(parsed);
      if (!keyPhrases.length) throw new Error('Empty result');

      // Normalize word coverage
      const currentTotal = keyPhrases.reduce((s, k) => s + k.wordsCovered, 0);
      const ratio = totalWords / currentTotal;

      keyPhrases = keyPhrases.map(k => ({
        ...k,
        wordsCovered: Math.max(1, Math.round(k.wordsCovered * ratio)),
      }));

      // Final correction
      const finalDiff =
        totalWords - keyPhrases.reduce((s, k) => s + k.wordsCovered, 0);
      if (finalDiff !== 0) {
        keyPhrases[keyPhrases.length - 1].wordsCovered += finalDiff;
      }

      return {
        keywords: keyPhrases.map(k => k.phrase),
        keyPhrases,
        clipCount: keyPhrases.length,
        audioDuration,
        totalWordsCovered: totalWords,
      };
    } catch (err) {
      console.error('AI extraction failed, using fallback:', err);
      return this.fallbackExtraction(narration, audioDuration, totalWords);
    }
  }
  ...
  private fallbackExtraction(
    ...
    const clipCount = Math.max(10, Math.ceil(audioDuration / 3));
    const wordsPer = Math.floor(totalWords / clipCount);
    const remainder = totalWords % clipCount;
    ...
    const keyPhrases: KeyPhrase[] = top.map((w, i) => ({
      phrase: `${w} scene footage`,
      wordsCovered: wordsPer + (i < remainder ? 1 : 0),
    }));
