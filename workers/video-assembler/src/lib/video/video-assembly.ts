...
import { applySfxToScene, SoundEvent } from '../assests/sfx-mixer';
...
export interface VideoAssemblyInput {
    ...
    perSceneSoundEvents?: SoundEvent[][];  // Per-scene animation sound events (from AI render mode)
    music?: string;
    branding?: {
        logo?: string;
        intro?: string;
...
export class VideoAssemblyService {
    ...
    async assembleVideo(input: VideoAssemblyInput): Promise<VideoAssemblyResult> {
        ...
        for (let i = 0; i < input.clips.length; i++) {
            ...
            // If we have sound events for this scene, mix SFX into the narration first
            const sceneSoundEvents: SoundEvent[] = input.perSceneSoundEvents?.[i] ?? [];
            let finalSceneAudio = sceneAudio;

            if (sceneSoundEvents.length > 0) {
                console.error(`  🎶 Applying ${sceneSoundEvents.length} SFX event(s) to scene ${i + 1}...`);
                const sfxLayerPath = path.join(outputDir, `sfx_layer_${i}.wav`);
                const sfxMixedAudio = path.join(outputDir, `scene_audio_sfx_${i}.wav`);

                try {
                    const sfxApplied = await applySfxToScene(
                        sceneSoundEvents,
                        sceneAudio,
                        targetDuration,
                        sfxLayerPath,
                    ...
                    if (sfxApplied) {
                        finalSceneAudio = sfxMixedAudio;
                        console.error(`  ✅ SFX mixed into scene ${i + 1} audio`);
                    }
                } catch (sfxErr) {
                    console.error(`  ⚠️  SFX mixing failed for scene ${i + 1} (non-fatal, using narration only):`, sfxErr);
                } finally {
                    // Cleanup sfx layer temp file
                    await fsPromises.unlink(sfxLayerPath).catch(() => {});
                }
            }
            ...
            await Promise.allSettled([
                ...
                // Also cleanup the sfx-mixed audio if it was created
                ...(finalSceneAudio !== sceneAudio
                    ? [fsPromises.unlink(finalSceneAudio)]
                    : []),
            ]);
        }
