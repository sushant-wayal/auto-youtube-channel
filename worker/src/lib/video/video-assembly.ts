...
export class VideoAssemblyService {
    ...
    async assembleVideo(input: VideoAssemblyInput): Promise<VideoAssemblyResult> {
        ...
        const mainClipsVideo = await this.normalizeAndConcatClipsWithTiming(input.clips, clipTimings, outputDir, input.isShort);
    ...
    async normalizeAndConcatClipsWithTiming(
        ...
        for (let i = 0; i < clips.length; i++) {
            ...
            const clipPath = path.join(normalizedDir, `clip_${i + 1}.mp4`);

            console.log(`  Downloading clip ${i + 1}/${clips.length} from ${clipUrl}...`);
            const response = await fetch(clipUrl);
            if (!response.ok) {
                throw new Error(`Failed to download clip from ${clipUrl}`);
            }

            const fileStream = fs.createWriteStream(clipPath);
            await new Promise((resolve, reject) => {
                if (!response.body) {
                    throw new Error(`Response body is null for clip ${clipUrl}`);
                }
                ...
                response.body?.getReader().read().catch(reject);
